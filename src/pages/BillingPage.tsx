            ...
            {records.map((r) => {
              ...
              return (
                <tr key={r.id} className={busy === r.id ? 'opacity-60' : ''}>
                  ...
                  <td className="px-3 py-2 text-right whitespace-nowrap">{dirty && <button className="text-xs bg-teal-700 text-white rounded px-2 py-1 mr-1" disabled={busy === r.id} onClick={() => saveNumbers(r)}>Save</button>}<button className="text-xs text-teal-700 hover:underline" onClick={() => copy(r.id, r.description)}>{copied === r.id ? 'Copied!' : 'Invoice text'}</button><button className="text-xs text-red-600 hover:underline ml-2" disabled={busy === r.id} onClick={() => removeRow(r)}>Delete</button></td>
                </tr>
              );
            })}
